...
int main()
{
	...
	*/
	...
	for (int n = 0; n < ImageNum; n++)
	{
		for (int c = 0; c < 10; c++) {
			...
			dy[n][c] = (TargetY[n][c] - OutSoft[n][c][0][0]);
	...
	*/
	...
	cudnnSoftmaxBackward(cudnn, CUDNN_SOFTMAX_ACCURATE, /*****이부분 매우 중요*****/CUDNN_SOFTMAX_MODE_CHANNEL, &alpha,
	...
	for (int n = 0; n < 5; n++)
	{
		for (int i = 0; i < 10; i++) {
			...
			std::cout << SoftBack[n][i][0][0] << " <- dx  ::   " << OutSoft[n][i][0][0] << " <- yData  ::   "  << "  ::   dy -> " << dy[n][i]  << std::endl;
		}
		std::cout << std::endl;
	}
	...
	//CPU에 결과 복사
	cudaMemcpy(FCBack, dev_Filter_Gradient, sizeof(float) * 3 * 10 * 4 * 4, cudaMemcpyDeviceToHost);
