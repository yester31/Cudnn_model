...
__global__ void Adam(
	...
	const size_t Height,
	const size_t Width,
	float *mt, float *vt, float *deltaW, float *delta,
	float LearningLate, double eps, float b1, float b2, int iter)
{
	...
	mt[idx] = (b1 * mt[idx] + (1 - b1) * delta[idx]);

	vt[idx] = (b2 * vt[idx] + (1 - b2) * delta[idx] * delta[idx]);


	deltaW[idx] = (-LearningLate * mt[idx] / (1 - pow(b1, iter))) / sqrt(vt[idx] / (1 - pow(b2, iter)) + eps);
...
int main()
{
	...
	for (int iter = 1; iter < epoch+1; iter++)
	{
		...
		for (int a = 0; a < ImageNum / batchSize; a++) { // 배치 계산 루프 
			...
			Adam << <(lastLayer_numOut * fc2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (lastLayer_numOut, fc2FilterCnt, 1, 1, fc3mt, fc3vt, fc3deltaw, dev_bwf_fc3_Output, learning_rate, eps, b1, b2, iter);

			cublasSaxpy(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), &eta, fc3deltaw, 1, dev_fc3_Filter, 1);

			Adam << <(1 * lastLayer_numOut * 1 * 1 + BW - 1) / BW, BW >> > (1, lastLayer_numOut, 1, 1, fc3Biasmt, fc3Biasvt, fc3BiasDeltaw, dev_bw_fc3Bias, learning_rate, eps, b1, b2, iter);

			cublasSaxpy(cublasHandle, static_cast<int>(1 * lastLayer_numOut * 1 * 1), &eta, fc3BiasDeltaw, 1, dev_fc3_Bias, 1);

			// Fully connected 2
			Adam << <(fc2FilterCnt * fc1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (fc2FilterCnt, fc1FilterCnt, 1, 1, fc2mt, fc2vt, fc2deltaw, dev_bwf_fc2_Output, learning_rate, eps, b1, b2, iter);
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &eta, fc2deltaw, 1, dev_fc2_Filter, 1);

		
			Adam << <(1 * fc2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, fc2FilterCnt, 1, 1, fc2Biasmt, fc2Biasvt, fc2BiasDeltaw, dev_bw_fc2Bias, learning_rate, eps, b1, b2, iter);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * fc2FilterCnt * 1 * 1), &eta, fc2BiasDeltaw, 1, dev_fc2_Bias, 1);

			// Fully connected 1
			Adam << <(fc1FilterCnt * conv2FilterCnt * pool2OutHeight * pool2OutWidth + BW - 1) / BW, BW >> > (fc1FilterCnt, conv2FilterCnt, pool2OutHeight, pool2OutWidth, fc1mt, fc1vt, fc1deltaw, dev_bwf_fc1_Output, learning_rate, eps, b1, b2, iter);
			...
			Adam << <(1 * fc1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, fc1FilterCnt, 1, 1, fc1Biasmt, fc1Biasvt, fc1BiasDeltaw, dev_bw_fc1Bias, learning_rate, eps, b1, b2, iter);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * fc1FilterCnt * 1 * 1), &eta, fc1BiasDeltaw, 1, dev_fc1_Bias, 1);

			// Conv2
			Adam << <(conv2FilterCnt * conv1FilterCnt * conv2FilterHeight * conv2FilterWidth + BW - 1) / BW, BW >> > (conv2FilterCnt, conv1FilterCnt, conv2FilterHeight, conv2FilterWidth, conv2mt, conv2vt, conv2deltaw, dev_bwf_conv2_Output, learning_rate, eps, b1, b2, iter);
			...
			Adam << <(1 * conv2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, conv2FilterCnt, 1, 1, conv2Biasmt, conv2Biasvt, conv2BiasDeltaw, dev_bw_conv2Bias, learning_rate, eps, b1, b2, iter);

			cublasSaxpy(cublasHandle, static_cast<int>(1 * conv2FilterCnt * 1 * 1), &eta, conv2BiasDeltaw, 1, dev_conv2_Bias, 1);

			// Conv1
			Adam << <(conv1FilterCnt * input_channelCnt * conv1FilterHeight * conv1FilterWidth + BW - 1) / BW, BW >> > (conv1FilterCnt, input_channelCnt, conv1FilterHeight, conv1FilterWidth, conv1mt, conv1vt, conv1deltaw, dev_bwf_conv1_Output, learning_rate, eps, b1, b2, iter);
			...
			Adam << <(1 * conv1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, conv1FilterCnt, 1, 1, conv1Biasmt, conv1Biasvt, conv1BiasDeltaw, dev_bw_conv1Bias, learning_rate, eps, b1, b2, iter);
	...
	cout << " b1 (adam)                  :: " << setw(10) << b1 << endl;
	cout << " b2 (adam)                  :: " << setw(10) << b2 << endl;
	cout << " eps (adam)                 :: " << setw(10) << eps << endl;
	cout << " Optimizer                  :: " << setw(10) << "Adam" << endl;
