...
int main()
{
	...
	cudnnBatchNormMode_t batchNormMode = CUDNN_BATCHNORM_PER_ACTIVATION; // 데이터 한건당 하나 1xCxHxW
	//cudnnBatchNormMode_t batchNormMode = CUDNN_BATCHNORM_SPATIAL;		// 채널 단위 1xCx1x1
	//cudnnBatchNormMode_t batchNormMode = CUDNN_BATCHNORM_SPATIAL_PERSISTENT;

	cudnnBatchNormOps_t batchNormOps = CUDNN_BATCHNORM_OPS_BN;
	//cudnnBatchNormOps_t batchNormOps = CUDNN_BATCHNORM_OPS_BN_ACTIVATION;
	//cudnnBatchNormOps_t batchNormOps = CUDNN_BATCHNORM_OPS_BN_ADD_ACTIVATION;
	...
	endTime_train = clock();
	printf("학습 계산 시간: %.1f 분 \n", (float)(endTime_train - startTime_train) / ((CLOCKS_PER_SEC) * 60));
	...
	std::cout << "=============== 학습 정보 ===============" << endl;
	cout << setw(10) << ImageNum << " 의 이미지들 학습 " << endl;
	cout << setw(10) << batchSize << " 배치 크기 " << endl;
	cout << setw(10) << epoch << " 회 학습 " << endl;
	cout << endl; cout << endl;

	std::cout << "=============== 테스트 정보 ===============" << endl;
	cout << setw(10) << ImageNum_test << " 의 이미지들 테스트 " << endl;
	cout << setw(10) << batch_size_test << " 배치 크기 " << endl;
	cout << endl; cout << endl;

	std::cout << "=============== 테스트 결과 ===============" << endl;
	cout << setw(10) << ImageNum_test << " 개의 이미지들 중 " << count << " 개 맞음" << endl;
