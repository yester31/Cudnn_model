	...
	do
	{
		...
		if (file_info.attrib & _A_SUBDIR)
		{
			if (file_name != "." && file_name != "..")
			{
				...
				//std::cout << "a sub_folder path: " << sub_folder_path << endl;
			}
		}
		else  //std::cout << "file name: " << file_name << endl;
		{
...
int main()
{
	...
	const int ImageNum = 100; // train 이미지 총 갯수
	const int batchSize = 10; // train 배치수
	const int ImageNum_test = 100; // test 이미지 수
	const int batch_size_test = 10; // test 배치수
	int epoch = 1;
	vector<pair<Mat, string>> ImgBox; // 이미지 데이터, 이미지 이름
	ImgBox = TraverseFilesUsingDFS("D:\\DataSet\\cifar\\test1");// 이미지가 저장되어 있는 폴더 경로
	...
	*/
	...
	float conv1Filter_delta[conv1FilterCnt][input_channelCnt][conv1FilterHeight][conv1FilterWidth];
	...
	float* dev_target;
	cudaMalloc((void**)&dev_target, batchSize * sizeof(float));
	...
	vector <float> Input_train;
	vector <float> target_train_batch;
	// train
	...
	for (int iter = 0; iter < epoch; iter++)
	{
		...
		for (int a = 0; a < ImageNum / batchSize; a++) { // 배치 계산 루프 
			...
			//Input_train.resize(0);
			for (int i = a*batchSize; i < batchSize*(a+1); i++)
			{
				...
				for (int c = 0; c < FeatureNum; c++)
				{
					for (int y = 0; y < FeatureHeight; y++)
					{
						...
						{
							Input_train.push_back(temp[3 * 32 * y + 3 * x + c]);
			...
			//target_train_batch.resize(0);
			for (int i = a*batchSize; i < batchSize*(a + 1); i++)
			{
				target_train_batch.push_back(target_train[i]);
			}


			cudaMemcpy(dev_target, &target_train_batch[0], ImageNum*num_labels * sizeof(float), cudaMemcpyHostToDevice);

			checkCudaErrors(cudaMemcpy(dev_input, &Input_train[0], sizeof(float) * batchSize * input_channelCnt * imageHeight * imageWidth, cudaMemcpyHostToDevice));
			//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			//GPU train 계산 들어갈 자리 
			//conv1 forward
			...
			cudaMemcpy(conv1Filter_delta, dev_bwf_fc3_Output, sizeof(float)*conv1FilterCnt * input_channelCnt * conv1FilterHeight * conv1FilterWidth, cudaMemcpyDeviceToHost);
																		
						
						for (int s = 0; s < conv1FilterWidth; s++) 
						{

							std::cout << conv1Filter_delta[0][0][0][s] << " ";
	...
	//vector <float> Input_test;
	//vector <float> target_test_batch;
	...
	int predicted[ImageNum_test]; // 소프트 맥스 결과 값에서 가장 큰 값이 들어 있는 위치(인덱스)를 저장하는 배열
	int count = 0;
	...
	for (int a = 0; a < ImageNum_test / batch_size_test; a++) { // 배치 계산 루프 
		...
		for (int i = batch_size_test*a; i < batch_size_test*(a + 1); i++)
		{
			...
			for (int c = 0; c < FeatureNum_test; c++)
			{
				for (int y = 0; y < FeatureHeight_test; y++)
				{
					...
					{
						//Input_test.push_back(temp[3 * 32 * x + 3 * y + c]);
						Input_test[index][c][y][x] = temp[3 * 32 * x + 3 * y + c];
					}
				}
			}
		}

		//target_test_batch.resize(0);
		...
		checkCudaErrors(cudaMemcpy(dev_input, Input_test, sizeof(float) * batch_size_test * input_channelCnt * imageHeight * imageWidth, cudaMemcpyHostToDevice));
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//GPU test 계산 들어갈 자리 
		//conv1 forward
		cudnnConvolutionForward(cudnn, &alpha, input_Tensor, dev_input, conv1_Filter_Desc, dev_conv1_Filter, conv1_Desc, conv1_fwAlgo, dev_conv1_works, conv1_worksSize, &beta, conv1_Tensor, dev_conv1_Output);
	...
	std ::cout << ImageNum_test << " 개의 이미지들 중 " << count << " 개 맞음" << endl;
	std::cout << "정확도 :: " << (count*1.0 / ImageNum_test) * 100 << " %" << endl;
	std::cout << "에러   :: " << (1 - (count*1.0 / ImageNum_test)) * 100 << " %" << endl;
