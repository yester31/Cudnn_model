	...
	int batchsize )
{
	...
	deltaAvg[idx] += delta[idx] / batchsize;

}


__global__ void BatchGradAvgreset(
	const size_t NextLayerNodeNumber,
	const size_t PrevLayerNodeNumber,
	const size_t Height,
	const size_t Width,
...
int main()
{
	...
	for (int iter = 1; iter < epoch + 1; iter++)
	{
		...
		for (int a = 0; a < ImageNum / batchSize; a++) { // 배치 계산 루프 
			...
			BatchGradAvg << <(lastLayer_numOut * fc2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (lastLayer_numOut, fc2FilterCnt, 1, 1, dev_bwf_fc3_Output, fc3vt, batchSize);
			BatchGradAvg << <(1 * lastLayer_numOut * 1 * 1 + BW - 1) / BW, BW >> > (1, lastLayer_numOut, 1, 1, dev_bw_fc3Bias, fc3Biasvt, batchSize);
			
			// Fully connected 2
			BatchGradAvg << <(fc2FilterCnt * fc1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (fc2FilterCnt, fc1FilterCnt, 1, 1, dev_bwf_fc2_Output, fc2vt, batchSize );
			BatchGradAvg << <(1 * fc2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, fc2FilterCnt, 1, 1, dev_bw_fc2Bias, fc2Biasvt, batchSize);
		
			// Fully connected 1
			BatchGradAvg << <(fc1FilterCnt * conv2FilterCnt * pool2OutHeight * pool2OutWidth + BW - 1) / BW, BW >> > (fc1FilterCnt, conv2FilterCnt, pool2OutHeight, pool2OutWidth,   dev_bwf_fc1_Output, fc1vt, batchSize);
			BatchGradAvg << <(1 * fc1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, fc1FilterCnt, 1, 1,   dev_bw_fc1Bias, fc1Biasvt, batchSize);

			// Conv2
			BatchGradAvg << <(conv2FilterCnt * conv1FilterCnt * conv2FilterHeight * conv2FilterWidth + BW - 1) / BW, BW >> > (conv2FilterCnt, conv1FilterCnt, conv2FilterHeight, conv2FilterWidth, dev_bwf_conv2_Output, conv2vt, batchSize);
			BatchGradAvg << <(1 * conv2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, conv2FilterCnt, 1, 1,  dev_bw_conv2Bias, conv2Biasvt, batchSize);

			// Conv1
			BatchGradAvg << <(conv1FilterCnt * input_channelCnt * conv1FilterHeight * conv1FilterWidth + BW - 1) / BW, BW >> > (conv1FilterCnt, input_channelCnt, conv1FilterHeight, conv1FilterWidth,  dev_bwf_conv1_Output, conv1vt, batchSize);
			BatchGradAvg << <(1 * conv1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, conv1FilterCnt, 1, 1, dev_bw_conv1Bias, conv1Biasvt, batchSize);
		...
		// Fully connected 3
		Adagrad << <(lastLayer_numOut * fc2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (lastLayer_numOut, fc2FilterCnt, 1, 1, fc3mt, fc3deltaw, fc3vt, learning_rate, eps);
		cublasSaxpy(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), &eta, fc3deltaw, 1, dev_fc3_Filter, 1);

		Adagrad << <(1 * lastLayer_numOut * 1 * 1 + BW - 1) / BW, BW >> > (1, lastLayer_numOut, 1, 1, fc3Biasmt, fc3BiasDeltaw, fc3Biasvt, learning_rate, eps);
		cublasSaxpy(cublasHandle, static_cast<int>(1 * lastLayer_numOut * 1 * 1), &eta, fc3BiasDeltaw, 1, dev_fc3_Bias, 1);

		// Fully connected 2
		Adagrad << <(fc2FilterCnt * fc1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (fc2FilterCnt, fc1FilterCnt, 1, 1, fc2mt, fc2deltaw, fc2vt, learning_rate, eps);
		cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &eta, fc2deltaw, 1, dev_fc2_Filter, 1);

		Adagrad << <(1 * fc2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, fc2FilterCnt, 1, 1, fc2Biasmt, fc2BiasDeltaw, fc2Biasvt, learning_rate, eps);
		cublasSaxpy(cublasHandle, static_cast<int>(1 * fc2FilterCnt * 1 * 1), &eta, fc2BiasDeltaw, 1, dev_fc2_Bias, 1);

		// Fully connected 1
		Adagrad << <(fc1FilterCnt * conv2FilterCnt * pool2OutHeight * pool2OutWidth + BW - 1) / BW, BW >> > (fc1FilterCnt, conv2FilterCnt, pool2OutHeight, pool2OutWidth, fc1mt, fc1deltaw, fc1vt, learning_rate, eps);
		...
		Adagrad << <(1 * fc1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, fc1FilterCnt, 1, 1, fc1Biasmt, fc1BiasDeltaw, fc1Biasvt, learning_rate, eps);
		cublasSaxpy(cublasHandle, static_cast<int>(1 * fc1FilterCnt * 1 * 1), &eta, fc1BiasDeltaw, 1, dev_fc1_Bias, 1);

		// Conv2
		Adagrad << <(conv2FilterCnt * conv1FilterCnt * conv2FilterHeight * conv2FilterWidth + BW - 1) / BW, BW >> > (conv2FilterCnt, conv1FilterCnt, conv2FilterHeight, conv2FilterWidth, conv2mt, conv2deltaw, conv2vt, learning_rate, eps);
		...
		Adagrad << <(1 * conv2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, conv2FilterCnt, 1, 1, conv2Biasmt, conv2BiasDeltaw, conv2Biasvt, learning_rate, eps);
		cublasSaxpy(cublasHandle, static_cast<int>(1 * conv2FilterCnt * 1 * 1), &eta, conv2BiasDeltaw, 1, dev_conv2_Bias, 1);

		// Conv1
		Adagrad << <(conv1FilterCnt * input_channelCnt * conv1FilterHeight * conv1FilterWidth + BW - 1) / BW, BW >> > (conv1FilterCnt, input_channelCnt, conv1FilterHeight, conv1FilterWidth, conv1mt, conv1deltaw, conv1vt, learning_rate, eps);
		...
		Adagrad << <(1 * conv1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, conv1FilterCnt, 1, 1, conv1Biasmt, conv1BiasDeltaw, conv1Biasvt, learning_rate, eps);
		...
		BatchGradAvgreset << <(lastLayer_numOut * fc2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (lastLayer_numOut, fc2FilterCnt, 1, 1, fc3vt);
		BatchGradAvgreset << <(1 * lastLayer_numOut * 1 * 1 + BW - 1) / BW, BW >> > (1, lastLayer_numOut, 1, 1, fc3Biasvt);
	
		// Fully connected 2
		BatchGradAvgreset << <(fc2FilterCnt * fc1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (fc2FilterCnt, fc1FilterCnt, 1, 1, fc2vt);
		BatchGradAvgreset << <(1 * fc2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, fc2FilterCnt, 1, 1, fc2Biasvt);

		// Fully connected 1
		BatchGradAvgreset << <(fc1FilterCnt * conv2FilterCnt * pool2OutHeight * pool2OutWidth + BW - 1) / BW, BW >> > (fc1FilterCnt, conv2FilterCnt, pool2OutHeight, pool2OutWidth, fc1vt);
		BatchGradAvgreset << <(1 * fc1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, fc1FilterCnt, 1, 1, fc1Biasvt);
		// Conv2
		BatchGradAvgreset << <(conv2FilterCnt * conv1FilterCnt * conv2FilterHeight * conv2FilterWidth + BW - 1) / BW, BW >> > (conv2FilterCnt, conv1FilterCnt, conv2FilterHeight, conv2FilterWidth, conv2vt);
		BatchGradAvgreset << <(1 * conv2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, conv2FilterCnt, 1, 1, conv2Biasvt);
		
		// Conv1
		BatchGradAvgreset << <(conv1FilterCnt * input_channelCnt * conv1FilterHeight * conv1FilterWidth + BW - 1) / BW, BW >> > (conv1FilterCnt, input_channelCnt, conv1FilterHeight, conv1FilterWidth, conv1vt);
		BatchGradAvgreset << <(1 * conv1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, conv1FilterCnt, 1, 1, conv1Biasvt);
		...
		cout << "# EPOCH (" << setw(3) << iter << " / " << epoch << " ) , " << "Train Set Accuracy ( " << setw(5) << (count*100.0 / ImageNum) << " %)" << " , 맞은 갯수 :: " << setw(5) << count << endl;
	...
	cout << " Batch Cal                  :: " << setw(10) << "Mini Batch" << endl;
