...
int main()
{
	...
	*/
	...
	for (int i = 0; i < 10; i++)
	{
		std::cout << setw(5) << OutSoft[0][i][0][0] << "  "; std::cout << std::endl;
	}
	...
	*/
	...
	/*
	vector<vector<int>> TargetY(1); // 임시 타켓 벡터 생성

									// 코드 확인을 위해 임시 타켓값 설정
	for (int n = 0; n < 9; n++)
	{
		TargetY[0].push_back({ 0 });
	}
	TargetY[0].push_back({ 1 });

	
	// 0000000001 값이 제대로 들어 갔는지 확인
	std::cout << std::endl << std::endl << "TargetY 값 확인" << std::endl << std::endl;
	for (int n = 0; n < 10; n++)
	{
	std::cout << TargetY[0][n];
	}
	std::cout << std::endl;

	// 결과값 로그 계산
	std::cout << std::endl << std::endl << "Log (OutSoft) 값" << std::endl << std::endl;

	for (int n = 0; n < 10; n++)
	{
	std::cout << -log(OutSoft[0][n][0][0]) << std::endl;
	}
	std::cout << std::endl;
	...
	for (int i = 0; i < 10; i++)
	{
		float aaa = OutSoft[n][i][0][0];

		std::cout << setw(5) << aaa << "  "; std::cout << std::endl;
	}
	}
	*/
