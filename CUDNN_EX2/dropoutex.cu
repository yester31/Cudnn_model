...
public:
	...
	Dropout(cudnnHandle_t cudnn, float dropR, int batchS, int feat, int imageH, int imageW) :
		dropRate(dropR), batchSize(batchS), features(feat), imgH(imageH), imgW(imageW)
	{
		...
		cudaMalloc(&d_dropout_out, sizeof(float)*batchSize*features*imgH*imgW);
		cudaMalloc(&d_dx_dropout, sizeof(float)*batchSize*features*imgH*imgW);
	};
