...
int main()
{
	...
	const int ImageNum = 100; // 이미지 총 갯수
	...
	*/
	...
	//입력변수
	const int batch_size_test = ImageNum_test;
	...
	int count = 0;
		...
		for (int i = 0; i < batch_size_test; i++)
		{
			...
			for (int c = 0; c < FeatureNum_test; c++)
			{
				for (int y = 0; y < FeatureHeight_test; y++)
				{
					...
					{
						Input_test[i][c][y][x] = temp[3 * 32 * x + 3 * y + c];
		...
		for (int i = 0; i < batch_size_test ; i++)
		{
			target_test_batch[i] = target_test[i];
		}
