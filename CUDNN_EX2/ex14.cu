...
int main()
{
	...
	const int ten = 10;
	//FC bias 결과 저장
	float Output_FC_Bias[ImageNum][ten][1][1];
	...
	cudnnSetTensor4dDescriptor(out_Bias_FC_desc, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, ImageNum, ten, 1, 1);
	...
	/*
	for (int i = 0; i < ten; i++) {
	std::cout << "FC + Bias ::  " << setw(8) << Output_FC_Bias[0][i][0][0];
	std::cout << "FC :: " << setw(8) << Output_FC[0][i][0][0];
	std::cout << "Bias :: " << setw(8) << biasValueFC[i];
	std::cout << std::endl;
	}
	*/
	...
	for (int n = 0; n < ImageNum; n++)
	{
		for (int c = 0; c < 10; c++) {
			...
			dy[n][c] = (TargetY[n][c] - OutSoft[n][c][0][0])*(-1);
	...
	for (int col = 0; col < 4; col++)
	{
	Weights2[n][ch][row][col] += Weights[och][ch][row][col]* dy[n][och];
	}}}}}
	...
	*/
	...
	cudnnConvolutionBackwardFilter(cudnn, &alpha,
		out_pool_desc, dev_Output_Pool,				 //  ImageNum, 3,	8,	8
		dif_soft_back, dev_Output_Soft_Back,		 //  ImageNum, 10 , 1,	1
		fc_desc, CUDNN_CONVOLUTION_BWD_FILTER_ALGO_0,
		dev_WS3, WS_size3, &beta, 
		weights_desc, dev_Filter_Gradient);			//  3 * 10 * 4 * 4

	//CPU에 결과 복사
	cudaMemcpy(FCdeltaW, dev_Filter_Gradient, sizeof(float) * 3 * 10 * 4 * 4, cudaMemcpyDeviceToHost);

	/*
	std::cout << std::endl << std::endl << "FCdeltaW" << std::endl << std::endl;
	...
	for (int n = 0; n < 10; n++)
	{
		for (int c = 0; c < 3; c++)
		{
			for (int i = 0; i < 4; i++)
			{
				for (int j = 0; j < 4; j++)
				{
					std::cout << setw(3) << FCdeltaW[n][c][i][j] << " :: ";
				}
				std::cout << std::endl;
			}
			std::cout << std::endl;
	...
	*/
	...
	//저장행렬 선언
	float FCBack[ImageNum][3][4][4]; // fc 바로 전 값 
	...
	cudnnConvolutionBackwardData(cudnn, 
		&alpha,
		weights_desc, dev_Filter_Gradient,
		dif_soft_back, dev_Output_Soft_Back,
		fc_desc, CUDNN_CONVOLUTION_BWD_DATA_ALGO_0,
		dev_WS3, WS_size3, 
		&beta,
		out_pool_desc, dev_data_Gradient);
	//CPU에 결과 복사
	cudaMemcpy(FCBack, dev_data_Gradient, sizeof(float) * 3 * ImageNum * 4 * 4, cudaMemcpyDeviceToHost);
	...
	for (int n = 0; n < 10; n++)
	{
		for (int c = 0; c < 3; c++)
		{
			for (int i = 0; i < 4; i++)
			{
				for (int j = 0; j < 4; j++)
				{
					std::cout << setw(3) << FCBack[n][c][i][j] << " :: ";
				}
				std::cout << std::endl;
			}
			std::cout << std::endl;
	...
	*/
	...
	float PoolingBack[ImageNum][3][8][8];
	...
	cudaMemcpy(PoolingBack, dev_Pool_Back, sizeof(float) *ImageNum * 3 * 8 * 8, cudaMemcpyDeviceToHost);
	/*
	...
	*/
	...
	cudaMemcpy(ActBack, dev_Act_Back, sizeof(float) *ImageNum * 3 * 8 * 8, cudaMemcpyDeviceToHost);
	...
	*/
	...
	//저장행렬
	float ConvbiasBack[ImageNum][3][1][1];
	...
	cudaMemcpy(FCbiasBack, dev_Conv_bias_Back, sizeof(float), cudaMemcpyDeviceToHost);
	...
	cudaMemcpy(Filter_conv, dev_Filter_conv2, sizeof(float) *3 * 3 * 8 * 8, cudaMemcpyDeviceToHost);
	...
	// 맞다 틀리든 모르지만 ....... 암튼  ConV delta Weight 까지 계산 결과 나옴 
