...
int main()
{
	...
	int ImageNum = 10000; // 이미지 총 개수
	int batchSize = 50; // 트레이닝 배치 수

	int ImageNum_test = 1400; // 이미지 총 개수
	const int batch_size_test = 50; // 테스트 배치 수

	int epoch = 100; // epoch 횟수 

	float learning_rate = 0.00006; // 학습률
	float momentum = 0.99;
	float eta = -1.0;
	...
	ImgBox = TraverseFilesUsingDFS("C:\\cifar\\MNIST_train");// 이미지가 저장되어 있는 폴더 경로
	...
	ImgBox_test = TraverseFilesUsingDFS("C:\\cifar\\MNIST_test");// 이미지가 저장되어 있는 폴더 경로
	...
	const int conv1FilterCnt = 4, conv1FilterHeight = 5, conv1FilterWidth = 5;

	int conv1Pad_h = 0, conv1Pad_w = 0, conv1Str_h = 1, conv1Str_w = 1, conv1Dil_h = 1, conv1Dil_w = 1;

	int conv2FilterCnt = 12, conv2FilterHeight = 5, conv2FilterWidth = 5;
	...
	int conv1OutHeight = 24;
	int conv1OutWidth = 24;

	int pool1OutHeight = 12;
	int pool1OutWidth = 12;

	int conv2OutHeight = 8;
	int conv2OutWidth = 8;

	int pool2OutHeight = 4;
	int pool2OutWidth = 4;
	...
	//fc1 filter
	float* fc1Filter = new float[lastLayer_numOut * conv2FilterCnt * pool2OutHeight * pool2OutWidth];
	InitWeightsXavier(fc1Filter, lastLayer_numOut, conv2FilterCnt, pool2OutHeight, pool2OutWidth);
	...
	//fc1 bias
	float* fc1Bias = new float[lastLayer_numOut];
	InitWeightsbias(fc1Bias, lastLayer_numOut);
	...
	checkCudaErrors(cudaMalloc((void**)&dev_fc1_Filter, sizeof(float) * lastLayer_numOut * conv2FilterCnt * pool2OutHeight * pool2OutWidth));
	checkCudaErrors(cudaMemcpy(dev_fc1_Filter, fc1Filter, sizeof(float) * lastLayer_numOut * conv2FilterCnt * pool2OutHeight * pool2OutWidth, cudaMemcpyHostToDevice));

	float* dev_fc1_Output;
	checkCudaErrors(cudaMalloc((void**)&dev_fc1_Output, sizeof(float) * batchSize * lastLayer_numOut * 1 * 1));

	float* dev_fc1_Bias;
	checkCudaErrors(cudaMalloc((void**)&dev_fc1_Bias, sizeof(float) * 1 * lastLayer_numOut * 1 * 1));
	checkCudaErrors(cudaMemcpy(dev_fc1_Bias, fc1Bias, sizeof(float) * 1 * lastLayer_numOut * 1 * 1, cudaMemcpyHostToDevice));

	float* dev_fc1_Actout;
	checkCudaErrors(cudaMalloc((void**)&dev_fc1_Actout, sizeof(float) * batchSize * lastLayer_numOut * 1 * 1));
	...
	checkCUDNN(cudnnSetFilter4dDescriptor(fc1_Filter_Desc, CUDNN_DATA_FLOAT, CUDNN_TENSOR_NCHW, lastLayer_numOut, conv2FilterCnt, pool2OutHeight, pool2OutWidth));
	...
	checkCUDNN(cudnnSetTensor4dDescriptor(fc1_biasTensor, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, 1, lastLayer_numOut, 1, 1));
	...
	checkCudaErrors(cudaMalloc((void**)&dev_bw_fc1_ActDelta, sizeof(float) * batchSize * lastLayer_numOut * 1 * 1));

	float* dev_bw_fc1Bias;
	checkCudaErrors(cudaMalloc((void**)&dev_bw_fc1Bias, sizeof(float) * 1 * lastLayer_numOut * 1 * 1));

	float* dev_bwf_fc1_Output;
	checkCudaErrors(cudaMalloc((void**)&dev_bwf_fc1_Output, sizeof(float) * lastLayer_numOut * conv2FilterCnt * pool2OutHeight * pool2OutWidth));
	...
	checkCudaErrors(cudaMalloc((void**)&dev_fc1FiltMentum, sizeof(float) * lastLayer_numOut * conv2FilterCnt * pool2OutHeight * pool2OutWidth));
	MomentumInitialize << <(lastLayer_numOut * conv2FilterCnt * pool2OutHeight * pool2OutWidth + BW - 1) / BW, BW >> > (lastLayer_numOut, conv2FilterCnt, pool2OutHeight, pool2OutWidth, dev_fc1FiltMentum);

	float* dev_fc1BiasMentum;
	checkCudaErrors(cudaMalloc((void**)&dev_fc1BiasMentum, sizeof(float) * 1 * lastLayer_numOut * 1 * 1));
	MomentumInitialize << <(1 * lastLayer_numOut * 1 * 1 + BW - 1) / BW, BW >> > (1, lastLayer_numOut, 1, 1, dev_fc1BiasMentum);
	...
	for (int iter = 0; iter < epoch; iter++)
	{
		...
		for (int a = 0; a < ImageNum / batchSize; a++) { // 배치 계산 루프 
			...
			cudnnActivationForward(cudnn, fc1_Act_Desc, &alpha, fc1_Tensor, dev_fc1_Output, &beta, fc1_Tensor, dev_smaxOutput);
			...
			cudnnConvolutionBackwardBias(cudnn, &alpha, fc1_Tensor, dev_dloss, &beta, fc1_biasTensor, dev_bw_fc1Bias);
			cudnnConvolutionBackwardFilter(cudnn, &alpha, pool2_Tensor, dev_conv2pool_Act_output, fc1_Tensor, dev_dloss, fc1_Desc,
				fc1_bwFAlgo, dev_fc1_bwFworks, fc1_bwFworksSize, &beta, fc1_Filter_Desc, dev_bwf_fc1_Output);
			cudnnConvolutionBackwardData(cudnn, &alpha, fc1_Filter_Desc, dev_fc1_Filter, fc1_Tensor, dev_dloss,
			...
			// Fully connected 1
			cublasSscal(cublasHandle, static_cast<int>(lastLayer_numOut * conv2FilterCnt * pool2OutHeight * pool2OutWidth), &momentum, dev_fc1FiltMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(lastLayer_numOut * conv2FilterCnt * pool2OutHeight * pool2OutWidth), &learning_rate, dev_bwf_fc1_Output, 1, dev_fc1FiltMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(lastLayer_numOut * conv2FilterCnt * pool2OutHeight * pool2OutWidth), &eta, dev_fc1FiltMentum, 1, dev_fc1_Filter, 1);

			cublasSscal(cublasHandle, static_cast<int>(1 * lastLayer_numOut * 1 * 1), &momentum, dev_fc1BiasMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * lastLayer_numOut * 1 * 1), &learning_rate, dev_bw_fc1Bias, 1, dev_fc1BiasMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * lastLayer_numOut * 1 * 1), &eta, dev_fc1BiasMentum, 1, dev_fc1_Bias, 1);
