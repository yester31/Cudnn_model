	...
	*/
	...
	for (int iter = 0; iter < 10; iter++)
	{
		...
		for (int a = 0; a < ImageNum / batchSize; a++) { // 배치 계산 루프 
			...
			for (int i = 0; i < batchSize; i++)
			{
				cout<< i << " :: "<< target_train_batch[i] << endl; ;
			}
			*/
			...
			cudaMemcpy(dev_target, target_train_batch, ImageNum*num_labels * sizeof(float), cudaMemcpyHostToDevice);

			checkCudaErrors(cudaMemcpy(dev_input, Input_train, sizeof(float) * batchSize * input_channelCnt * imageHeight * imageWidth, cudaMemcpyHostToDevice));
			//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			//GPU train 계산 들어갈 자리 
			//conv1 forward
			...
			SoftmaxLossBackprop << <(batchSize + 511) / 512, 512 >> > (dev_target, num_labels, batchSize, dev_dloss);
	...
	const int ImageNum_test = 1000; // 이미지 총 갯수
		...
		for (int i = batch_size_test*a; i < batch_size_test*(a + 1); i++)
		{
			...
			for (int c = 0; c < FeatureNum_test; c++)
			{
				for (int y = 0; y < FeatureHeight_test; y++)
				{
					...
					{
						Input_test[i - batch_size_test*a][c][y][x] = temp[3 * 32 * x + 3 * y + c];
