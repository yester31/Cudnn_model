...
int main()
{
	...
	for (int iter = 1; iter < epoch + 1; iter++)
	{
		...
		for (int a = 0; a < ImageNum / batchSize; a++) { // 배치 계산 루프 
			...
			RMSProp << <(lastLayer_numOut * fc2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (lastLayer_numOut, fc2FilterCnt, 1, 1, fc3mt, fc3deltaw, dev_bwf_fc3_Output, learning_rate, eps, b1);

			cublasSaxpy(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), &eta, fc3deltaw, 1, dev_fc3_Filter, 1);

			RMSProp << <(1 * lastLayer_numOut * 1 * 1 + BW - 1) / BW, BW >> > (1, lastLayer_numOut, 1, 1, fc3Biasmt, fc3BiasDeltaw, dev_bw_fc3Bias, learning_rate, eps, b1);

			cublasSaxpy(cublasHandle, static_cast<int>(1 * lastLayer_numOut * 1 * 1), &eta, fc3BiasDeltaw, 1, dev_fc3_Bias, 1);

			// Fully connected 2
			RMSProp << <(fc2FilterCnt * fc1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (fc2FilterCnt, fc1FilterCnt, 1, 1, fc2mt, fc2deltaw, dev_bwf_fc2_Output, learning_rate, eps, b1);
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &eta, fc2deltaw, 1, dev_fc2_Filter, 1);


			RMSProp << <(1 * fc2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, fc2FilterCnt, 1, 1, fc2Biasmt, fc2BiasDeltaw, dev_bw_fc2Bias, learning_rate, eps, b1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * fc2FilterCnt * 1 * 1), &eta, fc2BiasDeltaw, 1, dev_fc2_Bias, 1);

			// Fully connected 1
			RMSProp << <(fc1FilterCnt * conv2FilterCnt * pool2OutHeight * pool2OutWidth + BW - 1) / BW, BW >> > (fc1FilterCnt, conv2FilterCnt, pool2OutHeight, pool2OutWidth, fc1mt, fc1deltaw, dev_bwf_fc1_Output, learning_rate, eps, b1);
			...
			RMSProp << <(1 * fc1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, fc1FilterCnt, 1, 1, fc1Biasmt, fc1BiasDeltaw, dev_bw_fc1Bias, learning_rate, eps, b1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * fc1FilterCnt * 1 * 1), &eta, fc1BiasDeltaw, 1, dev_fc1_Bias, 1);

			// Conv2
			RMSProp << <(conv2FilterCnt * conv1FilterCnt * conv2FilterHeight * conv2FilterWidth + BW - 1) / BW, BW >> > (conv2FilterCnt, conv1FilterCnt, conv2FilterHeight, conv2FilterWidth, conv2mt, conv2deltaw, dev_bwf_conv2_Output, learning_rate, eps, b1);
			...
			RMSProp << <(1 * conv2FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, conv2FilterCnt, 1, 1, conv2Biasmt, conv2BiasDeltaw, dev_bw_conv2Bias, learning_rate, eps, b1);

			cublasSaxpy(cublasHandle, static_cast<int>(1 * conv2FilterCnt * 1 * 1), &eta, conv2BiasDeltaw, 1, dev_conv2_Bias, 1);

			// Conv1
			RMSProp << <(conv1FilterCnt * input_channelCnt * conv1FilterHeight * conv1FilterWidth + BW - 1) / BW, BW >> > (conv1FilterCnt, input_channelCnt, conv1FilterHeight, conv1FilterWidth, conv1mt, conv1deltaw, dev_bwf_conv1_Output, learning_rate, eps, b1);
			...
			RMSProp << <(1 * conv1FilterCnt * 1 * 1 + BW - 1) / BW, BW >> > (1, conv1FilterCnt, 1, 1, conv1Biasmt, conv1BiasDeltaw, dev_bw_conv1Bias, learning_rate, eps, b1);
	...
	cout << " Optimizer                  :: " << setw(10) << "RMSProp" << endl;
