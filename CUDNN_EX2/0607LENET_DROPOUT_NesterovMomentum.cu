...
int main()
{
	...
	float* dev_fc3FiltMentum2;
	checkCudaErrors(cudaMalloc((void**)&dev_fc3FiltMentum2, sizeof(float) * lastLayer_numOut * fc2FilterCnt * 1 * 1));
	...
	float* dev_fc2FiltMentum2;
	checkCudaErrors(cudaMalloc((void**)&dev_fc2FiltMentum2, sizeof(float) * fc2FilterCnt * fc1FilterCnt * 1 * 1));
	...
	float* dev_fc1FiltMentum2;
	checkCudaErrors(cudaMalloc((void**)&dev_fc1FiltMentum2, sizeof(float) * fc1FilterCnt * conv2FilterCnt * pool2OutHeight * pool2OutWidth));
	...
	float* dev_conv2FiltMentum2;
	checkCudaErrors(cudaMalloc((void**)&dev_conv2FiltMentum2, sizeof(float) * conv2FilterCnt * conv1FilterCnt * conv2FilterHeight * conv2FilterWidth));
	...
	float* dev_conv1FiltMentum2;
	checkCudaErrors(cudaMalloc((void**)&dev_conv1FiltMentum2, sizeof(float) * conv1FilterCnt * input_channelCnt * conv1FilterHeight * conv1FilterWidth));
	...
	vector<float> Accuracybox(ImageNum);
	int itera;
	for (int iter = 0; iter < epoch; iter++)
	{
		...
		for (int a = 0; a < ImageNum / batchSize; a++) { // 배치 계산 루프 
			...
			//learning_rate = static_cast<float>(learning_rate * pow(10, -(iter/2))); // 2 epoch 당 0.1 배 씩 계단식 감소 (step decay):
			//learning_rate = static_cast<float>(learning_rate * exp((-1)*iter*0.0001)); // 지수적 감소 (exponential decay)
			//learning_rate = static_cast<float>(learning_rate / (1 + iter*0.00001)); // 1/t 감소


			// Fully connected 3

			cublasScopy(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), dev_fc3FiltMentum, 1, dev_fc3FiltMentum2, 1); // mto - > mt2o  복사 
			cublasSscal(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), &momentum, dev_fc3FiltMentum, 1); // b1 * mto 행렬 곱 
			cublasSaxpy(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), &learning_rate, dev_bwf_fc3_Output, 1, dev_fc3FiltMentum, 1); // mtn == -lr*delta + b1*mt 행렬곱 하고 복사 
			cublasSscal(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), &momentum, dev_fc3FiltMentum2, 1); // b1 * mt2o
			
																																	//cublasSscal(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), &eta, dev_fc3FiltMentum2, 1); // - b1 * mt2o
			//cublasSscal(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), &momentum2, dev_fc3FiltMentum, 1); // b2 * mtn 행렬 곱 
			//cublasSaxpy(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), &eta2, dev_fc3FiltMentum, 1, dev_fc3FiltMentum2, 1); // w =  - b1 * mt2o + b2 * mtn 
			
			cublasSaxpy(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), &eta, dev_fc3FiltMentum2, 1, dev_fc3_Filter, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), &momentum2, dev_fc3FiltMentum, 1, dev_fc3_Filter, 1);
			...
			// Fully connected 2
			cublasScopy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), dev_fc2FiltMentum, 1, dev_fc2FiltMentum2, 1); // mto - > mt2o  복사 
			cublasSscal(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &momentum, dev_fc2FiltMentum, 1); // b1 * mto 행렬 곱 
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &learning_rate, dev_bwf_fc2_Output, 1, dev_fc2FiltMentum, 1); // mtn == -lr*delta + b1*mt 행렬곱 하고 복사 
			cublasSscal(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &momentum, dev_fc2FiltMentum2, 1); // b1 * mt2o
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &eta, dev_fc2FiltMentum2, 1, dev_fc2_Filter, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &momentum2, dev_fc2FiltMentum, 1, dev_fc2_Filter, 1);
			...
			// Fully connected 1
			cublasScopy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), dev_fc1FiltMentum, 1, dev_fc1FiltMentum2, 1); // mto - > mt2o  복사 
			cublasSscal(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &momentum, dev_fc1FiltMentum, 1); // b1 * mto 행렬 곱 
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &learning_rate, dev_bwf_fc1_Output, 1, dev_fc1FiltMentum, 1); // mtn == -lr*delta + b1*mt 행렬곱 하고 복사 
			cublasSscal(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &momentum, dev_fc1FiltMentum2, 1); // b1 * mt2o
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &eta, dev_fc1FiltMentum2, 1, dev_fc1_Filter, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &momentum2, dev_fc1FiltMentum, 1, dev_fc1_Filter, 1);
			...
			// Conv2
			cublasScopy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), dev_conv2FiltMentum, 1, dev_conv2FiltMentum2, 1); // mto - > mt2o  복사 
			cublasSscal(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &momentum, dev_conv2FiltMentum, 1); // b1 * mto 행렬 곱 
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &learning_rate, dev_bwf_conv2_Output, 1, dev_conv2FiltMentum, 1); // mtn == -lr*delta + b1*mt 행렬곱 하고 복사 
			cublasSscal(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &momentum, dev_conv2FiltMentum2, 1); // b1 * mt2o
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &eta, dev_conv2FiltMentum2, 1, dev_conv2_Filter, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &momentum2, dev_conv2FiltMentum, 1, dev_conv2_Filter, 1);
			...
			cublasScopy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), dev_conv1FiltMentum, 1, dev_conv1FiltMentum2, 1); // mto - > mt2o  복사 
			cublasSscal(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &momentum, dev_conv1FiltMentum, 1); // b1 * mto 행렬 곱 
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &learning_rate, dev_bwf_conv1_Output, 1, dev_conv1FiltMentum, 1); // mtn == -lr*delta + b1*mt 행렬곱 하고 복사 
			cublasSscal(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &momentum, dev_conv1FiltMentum2, 1); // b1 * mt2o
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &eta, dev_conv1FiltMentum2, 1, dev_conv1_Filter, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &momentum2, dev_conv1FiltMentum, 1, dev_conv1_Filter, 1);
		...
		itera = iter;
		Accuracybox[iter] = count*100.0 / ImageNum;
		float sum = 0.0;
		if (iter > 10) {

			sum = (int)Accuracybox[iter - 9] + (int)Accuracybox[iter - 8] + (int)Accuracybox[iter - 7] + (int)Accuracybox[iter - 6] + (int)Accuracybox[iter - 5]
				+ (int)Accuracybox[iter - 4] + (int)Accuracybox[iter - 3] + (int)Accuracybox[iter - 2] + (int)Accuracybox[iter - 1] + (int)Accuracybox[iter];;

			if ((int)sum / 10 == (int)Accuracybox[iter]) {
				cout << "정지 조건 발동" << endl;
	...
	cout << " 드라아웃(drop rate1)       :: " << setw(10) << dropRate1 << endl;
	cout << " 드라아웃(drop rate2)       :: " << setw(10) << dropRate2 << endl;
	cout << " 드라아웃(drop rate3)       :: " << setw(10) << dropRate3 << endl;
	cout << " 학습률 (learning_rate)     :: " << setw(10) << learning_rate << endl;
	cout << " 모멘텀 (momentum)          :: " << setw(10) << momentum << endl;
	cout << " Optimizer                  :: " << setw(10) << "Nesterov Momentum" << endl;

	cout << endl; cout << endl;

	std::cout << "=============== 테스트 정보 ===============" << endl;
	cout << " 테스트에 사용된 이미지 수   :: " << setw(10) << ImageNum_test << " 개" << endl;
	cout << " 배치 크기(batch size)       :: " << setw(10) << batch_size_test << " 개" << endl;
	cout << endl; cout << endl;
