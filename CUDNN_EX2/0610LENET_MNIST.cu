...
int main()
{
	...
	float learning_rate = -0.0001; // 학습률
	...
	for (int iter = 0; iter < epoch; iter++)
	{
		...
		for (int a = 0; a < ImageNum / batchSize; a++) { // 배치 계산 루프 
			...
			// Fully connected 1
			cublasSaxpy(cublasHandle, static_cast<int>(lastLayer_numOut * conv2FilterCnt * pool2OutHeight * pool2OutWidth), &learning_rate, dev_bwf_fc1_Output, 1, dev_fc1_Filter, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * lastLayer_numOut * 1 * 1), &learning_rate, dev_bw_fc1Bias, 1, dev_fc1_Bias, 1);
