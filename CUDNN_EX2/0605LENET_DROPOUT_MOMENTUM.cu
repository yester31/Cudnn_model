...
int main()
{
	...
	for (int iter = 0; iter < epoch; iter++)
	{
		...
		for (int a = 0; a < ImageNum / batchSize; a++) { // 배치 계산 루프 
			...
			//learning_rate = static_cast<float>(learning_rate * pow(10, -(iter/5))); // 5 epoch 당 0.1 배 씩 계단식 감소 (step decay):
			...
			*/
			...
			// Fully connected 3
			cublasSscal(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), &momentum, dev_fc3FiltMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), &eta, dev_bwf_fc3_Output, 1, dev_fc3FiltMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(lastLayer_numOut * fc2FilterCnt * 1 * 1), &learning_rate, dev_fc3FiltMentum, 1, dev_fc3_Filter, 1);

			cublasSscal(cublasHandle, static_cast<int>(1 * lastLayer_numOut * 1 * 1), &momentum, dev_fc3BiasMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * lastLayer_numOut * 1 * 1), &eta, dev_bw_fc3Bias, 1, dev_fc3BiasMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * lastLayer_numOut * 1 * 1), &learning_rate, dev_fc3BiasMentum, 1, dev_fc3_Bias, 1);

			// Fully connected 2
			cublasSscal(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &momentum, dev_fc2FiltMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &eta, dev_bwf_fc2_Output, 1, dev_fc2FiltMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(fc2FilterCnt * fc1FilterCnt * 1 * 1), &learning_rate, dev_fc2FiltMentum, 1, dev_fc2_Filter, 1);

			cublasSscal(cublasHandle, static_cast<int>(1 * fc2FilterCnt * 1 * 1), &momentum, dev_fc2BiasMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * fc2FilterCnt * 1 * 1), &eta, dev_bw_fc2Bias, 1, dev_fc2BiasMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * fc2FilterCnt * 1 * 1), &learning_rate, dev_fc2BiasMentum, 1, dev_fc2_Bias, 1);

			// Fully connected 1
			cublasSscal(cublasHandle, static_cast<int>(fc1FilterCnt * conv2FilterCnt * pool2OutHeight * pool2OutWidth), &momentum, dev_fc1FiltMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(fc1FilterCnt * conv2FilterCnt * pool2OutHeight * pool2OutWidth), &eta, dev_bwf_fc1_Output, 1, dev_fc1FiltMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(fc1FilterCnt * conv2FilterCnt * pool2OutHeight * pool2OutWidth), &learning_rate, dev_fc1FiltMentum, 1, dev_fc1_Filter, 1);

			cublasSscal(cublasHandle, static_cast<int>(1 * fc1FilterCnt * 1 * 1), &momentum, dev_fc1BiasMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * fc1FilterCnt * 1 * 1), &eta, dev_bw_fc1Bias, 1, dev_fc1BiasMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * fc1FilterCnt * 1 * 1), &learning_rate, dev_fc1BiasMentum, 1, dev_fc1_Bias, 1);

			// Conv2
			cublasSscal(cublasHandle, static_cast<int>(conv2FilterCnt * conv1FilterCnt * conv2FilterHeight * conv2FilterWidth), &momentum, dev_conv2FiltMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(conv2FilterCnt * conv1FilterCnt * conv2FilterHeight * conv2FilterWidth), &eta, dev_bwf_conv2_Output, 1, dev_conv2FiltMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(conv2FilterCnt * conv1FilterCnt * conv2FilterHeight * conv2FilterWidth), &learning_rate, dev_conv2FiltMentum, 1, dev_conv2_Filter, 1);

			cublasSscal(cublasHandle, static_cast<int>(1 * conv2FilterCnt * 1 * 1), &momentum, dev_conv2BiasMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * conv2FilterCnt * 1 * 1), &eta, dev_bw_conv2Bias, 1, dev_conv2BiasMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * conv2FilterCnt * 1 * 1), &learning_rate, dev_conv2BiasMentum, 1, dev_conv2_Bias, 1);

			// Conv1
			cublasSscal(cublasHandle, static_cast<int>(conv1FilterCnt * input_channelCnt * conv1FilterHeight * conv1FilterWidth), &momentum, dev_conv1FiltMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(conv1FilterCnt * input_channelCnt * conv1FilterHeight * conv1FilterWidth), &eta, dev_bwf_conv1_Output, 1, dev_conv1FiltMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(conv1FilterCnt * input_channelCnt * conv1FilterHeight * conv1FilterWidth), &learning_rate, dev_conv1FiltMentum, 1, dev_conv1_Filter, 1);

			cublasSscal(cublasHandle, static_cast<int>(1 * conv1FilterCnt * 1 * 1), &momentum, dev_conv1BiasMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * conv1FilterCnt * 1 * 1), &eta, dev_bw_conv1Bias, 1, dev_conv1BiasMentum, 1);
			cublasSaxpy(cublasHandle, static_cast<int>(1 * conv1FilterCnt * 1 * 1), &learning_rate, dev_conv1BiasMentum, 1, dev_conv1_Bias, 1);
			//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	cout << " 학습 회수(epoch)           :: " << setw(10) << itera+1 << " 회" << endl;
	...
	cout << " Optimizer                  :: " << setw(10) << "Momentum" << endl;
